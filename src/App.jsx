...
const App = () => {
    ...
    useEffect(() => {
        const fetchMetrics = async () => {
            try {
                ...
                if (email && latestImageData && latestImageData.count > capacity) {
    ...
    return (
        <div className="flex justify-center min-h-screen mx-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"style={{ backgroundColor: 'navy'}}>
            <div className="col-span-3 text-center">
                <h1 className="text-3xl font-bold mb-6"style={{ color: 'white' }}>Crowd Monitoring Dashboard</h1>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-md col-span-1">
                <label>Email:</label>
                <input type="email" placeholder="Enter your email" value={email} onChange={(e) => setEmail(e.target.value)} className="border p-2 rounded w-full" />
            </div>
            <div className="bg-white p-4 rounded-lg shadow-md col-span-1">
                <label>Mode:</label>
            ...
            <div className="bg-white p-4 rounded-lg shadow-md col-span-1">
                <h2 className="text-xl font-semibold">Highest Count in Last 24 Hours</h2>
                <p className="text-3xl font-bold mt-2">{highestCount}</p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-md col-span-1">
                <h2 className="text-xl font-semibold">Latest Image</h2>
                {latestImage ? <img src={latestImage} alt="Latest" className="rounded-lg mt-2 w-full" /> : <p className="text-gray-500 mt-2">No image available</p>}
            </div>
